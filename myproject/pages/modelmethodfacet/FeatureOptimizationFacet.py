...
from myproject.pages import pages_utils
from myproject.pages.modelandmethod.FeatureOptimizationMethod import FeatureOptimizationMethod
...
def onPreviewResults():
    ...
    if st.button("添加处理", on_click=clear_all):
        ...
        print('======================特征优选-添加任务清单记录======================')
        print(new_data)
        pages_utils.TempDataSetField[3].loc[len(pages_utils.TempDataSetFieldFacet[3])] = new_data
        st.rerun()
...
with dataPCV:
        ...
        with placeholder1.container():
            ...
            for i in range(len(tempLeftTabs)):
                with tt1[i]:
                    ...
                    st.data_editor(
                        pages_utils.TempDataSetFieldFacet[i + 2],
    ...
    weatherNameT, plantNameT, agricultureNameT = pages_utils.TempDataSetFacet[2].columns.tolist(), ['无1'], ['无2']
    # 数组元素去重
    ...
    st.checkbox('全选-地理遥感数据', disabled=True)
    result2 = []
    result3 = []
    ...
    if st.session_state.page14 == 0:
        ...
        with placeholder.container():
            ...
            with interval_col33:
                ...
                #         st.markdown('保留字段选择')
                #         residualField = [arr for arr in pages_utils.TempDataSetFacet[2].columns if
        ...
        with placeholder.container():
            ...
            # # idFMethods = pages_utils.TempDataSetFieldFacet[2]["特征优选方法"].tolist()
            #
            # # 若无方法处理,则直接跳过该环节
            # if len(idFMethods):
            #     # 创建新的从 1 开始的编号列表
