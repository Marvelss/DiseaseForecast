...
with hideBtnBrief.container():
    ...
    st.markdown('##### &emsp;&emsp;本次使用<u>点状数据</u>'
                '构建的<u>动态模型</u>场景为<u>水稻纹枯病峰值模型</u>,', unsafe_allow_html=True)
...
st.markdown('##### &emsp;&emsp;本实施例基于气象多情景仿真器输出的<u>高温多雨情景</u>、<u>低温少雨情景</u>数据与'
            '常温常雨情景数据对某茶小绿叶蝉虫害预测模型进行评价，该模型为动态预'
            '测模型，使用动态预测模型评价方法对其进行评价，模型输出为病害压力值'
            '及等级，不同情景下茶小绿叶蝉虫害预测模型预测的2010年茶小绿叶蝉虫害\n'
            '##### &emsp;&emsp;计算可得常温常雨情景下压力值预测输出的动态偏差指标Dev_D 压力值＝0.'
            '0047，模型等级预测输出的动态偏差指标Dev_D 等级＝-0.0359；高温多雨情'
            ...
            '影响，高温多雨情景下虫害发生期在2月和3月略有延长，该情景下模型动态'
            '偏差指标均呈现正向偏移，整体预测虫害压力值高于普通常温常雨情景；低'
            '温少雨情景下虫害发生期在3月略有缩短，模型动态偏差指标均呈现负向偏'
            '移，压力值低于普通常温常雨情景。预测结果与病虫害习性较为相符，且在'
            '多次模拟过程中结果趋势均保持较高程度的一致，模型输出较为合理。\n'
            '##### &emsp;&emsp;根据上述计算结果进行综合评估，可认为该模型可靠性较高，参数设置较为'
